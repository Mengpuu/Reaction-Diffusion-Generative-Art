...
float rand(int seed) {
    float x = sin(float(seed) * 12.9898 + 78.233 + iTime) * 43758.5453; // 
...
void mainImage( out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if(iTime < 0.5)
    {
        vec2 toMid = (fragCoord - iResolution.xy * 0.5) / iResolution.y;

        float sideLengthRatio = 0.5;  // 

        float halfSideLength = sideLengthRatio * 0.5;

        float initVal = step(abs(toMid.x), halfSideLength) * step(abs(toMid.y), halfSideLength);
    ...
    if(iTime < 0.5)
    {
        ...
        float radius = 0.1;  // screen height 10per cent

        float initVal = step(midDistSq, radius * radius);
    ...
    for (int i = 0; i < int(numLines); i++) {
        ...
        float dist = abs(dot(offset, lineDir));  // 

        initVal = max(initVal, step(dist, lineWidth)); // 
    }
    ...
    if (iTime < 0.5)
    {
        ...
        float radiusMin = 0.005;  // 
        float radiusMax = 0.015;  //

        float initVal = 0.0;
        for (int i = 0; i < int(numCircles); i++) {
            vec2 randomPos = vec2(rand(i + int(iTime * 100.0)), rand(i + int(iTime * 200.0)));  
            randomPos = (randomPos * 2.0 - 1.0) * 0.5;  //

            vec2 offset = fragCoord / iResolution.xy - 0.5;  
            float distSq = dot(offset - randomPos, offset - randomPos);

            float radius = mix(radiusMin, radiusMax, rand(i + 2));  

            initVal = max(initVal, step(distSq, radius * radius)); 
        }
